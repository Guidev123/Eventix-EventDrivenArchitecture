...
{
    internal sealed class UsersIdempotentIntegrationEventHandler<TIntegrationEvent>(
            ...
            where TIntegrationEvent : IIntegrationEvent
    {
        public override async Task ExecuteAsync(TIntegrationEvent integrationEvent, CancellationToken cancellationToken = default)
        {
            ...
            if (await IsInboxMessageProcessedAsync(inboxMessageConsumer, connection, Schemas.Users)) return;

            await innerHandler.ExecuteAsync(integrationEvent, cancellationToken);

            await MarkInboxMessageAsProcessedAsync(inboxMessageConsumer, connection, Schemas.Users);
        }
    }
}