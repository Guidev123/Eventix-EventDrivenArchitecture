...
{
    internal sealed class TicketingIdempotentDomainEventHandler<TDomainEvent>(
            ...
            where TDomainEvent : IDomainEvent
    {
        public override async Task ExecuteAsync(TDomainEvent domainEvent, CancellationToken cancellationToken = default)
        {
            ...
            if (await IsOutboxMessageProcessedAsync(outboxMessageConsumer, connection, Schemas.Ticketing)) return;

            await innerHandler.ExecuteAsync(domainEvent, cancellationToken);

            await MarkOutboxMessageAsProcessedAsync(outboxMessageConsumer, connection, Schemas.Ticketing);
        }
    }
}